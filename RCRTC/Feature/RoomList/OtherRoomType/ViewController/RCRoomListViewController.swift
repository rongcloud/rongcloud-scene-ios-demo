...
final class RCRoomListViewController: UIViewController {
    ...
    var audienceCDNType: RCSCDNType = .CDN_WS
...
extension RCRoomListViewController {
    @objc private func longPressedHandler(_ gesture: UILongPressGestureRecognizer) {
        ...
        let RTCAction = UIAlertAction(title: "MCU", style: .default) { _ in
            self.audienceCDNType = .MCU
        }
        let CDNAction = UIAlertAction(title: "CDN", style: .default) { _ in
            self.audienceCDNType = .CDN_WS
        }
        let BSYAction = UIAlertAction(title: "BSY", style: .default) { _ in
            self.audienceCDNType = .CDN(RCSLiveThirdCDN.shared)
        }
        let cancelAction = UIAlertAction(title: "取消", style: .cancel)
        controller.addAction(RTCAction)
        controller.addAction(CDNAction)
        if isBSYEnable() { controller.addAction(BSYAction) }
        controller.addAction(cancelAction)
        present(controller, animated: true)
    }
    
    private func isBSYEnable() -> Bool {
        guard
            let BSY = AppConfigs.ENV.CDN["BSY"],
            let pull = BSY["pull"],
            let push = BSY["push"]
        else { return false }
        return pull.count > 0 && push.count > 0
    }
}
