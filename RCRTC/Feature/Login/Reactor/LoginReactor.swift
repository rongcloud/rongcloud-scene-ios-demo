            ...
            .flatMapFirst({ user -> Observable<Result<User, ReactorError>> in
                return Observable<Result<User, ReactorError>>
                    .create { observer -> Disposable in
                        ...
                        let rcsc_User = RCSCUser.init(userId: user.userId, userName: user.userName, portrait: user.portrait, imToken: user.imToken, authorization: user.authorization, type: user.type, sex: 1) //FIXME:这里先默认sex,后期拿到可以重新定义接口
