...
struct AppENV: Codable {
    ...
    let third_cdn: [String: [String: String]]
}
...
class AppConfigs {
    ...
    static func thirdCDNHost(isPush: Bool) -> String {
        guard let BSY = ENV.third_cdn["bsy"] else { return "" }
        let push_host = BSY["push"] ?? ""
        let pull_host = BSY["pull"] ?? ""
        return isPush ? push_host : pull_host
    }
}
