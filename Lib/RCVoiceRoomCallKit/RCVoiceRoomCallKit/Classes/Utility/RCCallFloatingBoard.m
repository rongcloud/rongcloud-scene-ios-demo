...
- (void)updateActiveTimer {
    ...
    if (sec > 240 * 60 * 60) return;
    if (self.callSession.callStatus == RCCallActive && ![self isVideoViewEnabledSession]) {
        self.titleLabel.text = [RCCallKitUtility getReadableStringForTime:sec];
...
- (void)updateBoard {
    ...
    switch (self.callSession.callStatus) {
        case RCCallDialing:
            self.titleLabel.text = @"等待接听";
            break;
        case RCCallIncoming:
        case RCCallRinging:
            self.titleLabel.text = @"邀请通话";
            break;
        default:
            break;
    }
...
- (UIWindow *)window {
    if (!_window) {
        ...
        _window.backgroundColor = [UIColor colorWithRed:241 / 255.0 green:242 / 255.0 blue:245 / 255.0 alpha:1];
        _window.windowLevel = UIWindowLevelAlert + 1;
        _window.layer.cornerRadius = 8;
        _window.layer.masksToBounds = YES;
        _window.layer.borderColor = [UIColor colorWithRed:206 / 255.0 green:209 / 255.0 blue:212 / 255.0 alpha:1].CGColor;
        _window.layer.borderWidth = 1;
        _window.layer.shadowColor = [UIColor colorWithRed:32 / 255.0 green:34 / 255.0 blue:37 / 255.0 alpha:0.12].CGColor;
        _window.layer.shadowOffset = CGSizeMake(0, 4);
        _window.layer.shadowRadius = 8;
...
- (UIButton *)floatingButton {
    if (!_floatingButton) {
        ...
        if (self.callSession.mediaType == RCCallMediaAudio) {
            [_floatingButton setImage:nil forState:UIControlStateNormal];
        } else {
        ...
        _floatingButton.frame = CGRectMake(0, 0, 78, 78);
        CGRect windowFrame = self.window.frame;
        ...
        [self.window addSubview:self.statusView];
        self.titleLabel.text = @"等待接听";
    }
...
- (UIView *)statusView {
    if (_statusView == nil) {
        UIView *view = [[UIView alloc] initWithFrame:CGRectMake(8, 8, 62, 62)];
        view.backgroundColor = [UIColor whiteColor];
        view.layer.cornerRadius = 6;
        view.layer.masksToBounds = YES;
        view.userInteractionEnabled = NO;
        
        UIImage *image = [RCCallKitUtility imageFromVoIPBundle:@"voip/audio_call.png"];
        UIImageView *imageView = [[UIImageView alloc] initWithImage:image];
        imageView.frame = CGRectMake(17, 6, 28, 28);
        [view addSubview:imageView];
        
        [view addSubview:self.titleLabel];
        self.titleLabel.frame = CGRectMake(0, 39, 62, 16);
...
- (UILabel *)titleLabel {
    if (!_titleLabel) {
        ...
        _titleLabel.textColor = [UIColor colorWithRed:23 / 255.0 green:204 / 255.0 blue:132 / 255.0 alpha:1];
...
- (void)callDidDisconnect {
    ...
    [[[UIApplication sharedApplication] keyWindow] toastWhenDisconnect:self.callSession.disconnectReason];
}
