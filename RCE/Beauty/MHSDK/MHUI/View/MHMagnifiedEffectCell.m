...
- (void)setModel:(MHBeautiesModel *)model {
    ...
    if (model.menuType == MHBeautyMenuType_Magnify) {
        ...
        NSString *magnifyTitle = YZMsg(model.beautyTitle);
        ...
        ///修改MHUI
        _effectBtn.frame = CGRectMake((CGRectGetWidth(self.frame) - 55)/2, (CGRectGetHeight(self.frame) - 60)/2,  55, 60);
...
- (void)switchBtnSelectedStatus:(BOOL)isSelected {
    if (isSelected) {
        ...
        [self.effectBtn setTitleColor:FontColorBlackNormal forState:0];
