    ...
    func mutate(action: Action) -> Observable<Mutation> {
        ...
        case .clickSendVerifyCode:
            ...
            guard currentState.phoneNumber.count > 0 else {
                return Observable<Mutation>.just(.setError(ReactorError("请先输入手机号")))
            }
            let sendRequest = networkProvider.rx.request(.sendCode(mobile: currentState.phoneNumber)).map(SendCodeResponse.self).flatMap({ (response) -> Single<Mutation> in
                if response.validate() {
                    return Single<Mutation>.just(.setSuccess(ReactorSuccess("发送验证码成功")))
                } else {
                    return Single<Mutation>.just(.setError(ReactorError("发送验证码失败")))
                }
            }).asObservable().catchAndReturn(networkError())
            ...
            return Observable.concat([sendRequest, setCountdownBegin, timer, setCountdownClose, setTitle, reset])
        }
    }
