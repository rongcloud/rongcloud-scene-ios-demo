...
class DelegateImpl: NSObject, RCMusicEngineDelegate {
    
    static let instance = DelegateImpl()
    ...
    func downloadedMusic(_ music: RCMusicInfo, completion: @escaping (Bool) -> Void) {
    ...
    func deleteMusic(_ music: RCMusicInfo, completion: @escaping (Bool) -> Void) {
        ...
        DispatchQueue.global().async {
            let wait = self.semaphore.wait(timeout: .distantFuture)
            if (wait == .success) {
                let api: RCNetworkAPI = .deleteMusic(roomId: roomId, musicId: id)
                networkProvider.request(api) { result in
                    switch result.map(AppResponse.self) {
                    case .success:
                        completion(true)
    ...
    func topMusic(_ music1: RCMusicInfo?, withMusic music2: RCMusicInfo, completion: @escaping (Bool) -> Void) {
        ...
        DispatchQueue.global().async {
            let wait = self.semaphore.wait(timeout: .distantFuture)
            if (wait == .success) {
                let api: RCNetworkAPI = .moveMusic(roomId: roomId, fromId: id2, toId: id1 ?? 0)
                networkProvider.request(api) { result in
                    switch result.map(AppResponse.self) {
                    case .success:
    ...
    func syncPlayingMusicInfo(_ info: MusicInfo?) {
        ...
        let api: RCNetworkAPI = .syncRoomPlayingMusicInfo(roomId: roomId, musicId: id)
        networkProvider.request(api) { result in
            switch result.map(AppResponse.self) {
            case .success:
                log.debug("同步音乐信息成功")
