...
enum PasswordViewType {
    case input
    case verify(VoiceRoom)
}

protocol VoiceRoomInputPasswordProtocol: AnyObject {
    func passwordDidEnter(password: String)
    func passwordDidVarify(_ room: VoiceRoom)
}

extension VoiceRoomInputPasswordProtocol {
    func passwordDidEnter(password: String) {}
    func passwordDidVarify(_ room: VoiceRoom) {}
}

class VoiceRoomPasswordViewController: UIViewController {
    weak var delegate: VoiceRoomInputPasswordProtocol?
    ...
    init(type: PasswordViewType, delegate: VoiceRoomInputPasswordProtocol?) {
...
extension Reactive where Base == VoiceRoomPasswordViewController {
    var password: Observable<String> {
        return base.uploadButton.rx.tap.withLatestFrom(base.textField.rx.text.orEmpty.asObservable().filter { $0.count == 4 }.asObservable())
    }
}
