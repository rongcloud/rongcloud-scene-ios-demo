...
protocol OwnerSeatViewProtocol: AnyObject {
    func ownerSeatViewDidClick()
}

class OwnerSeatView: UIView {
    weak var delegate: OwnerSeatViewProtocol?
    ...
    @objc private func handleUserTap() {
        delegate?.ownerSeatViewDidClick()
    }
    
    public func updateOwner(seatInfo: RCVoiceSeatInfo) {
    ...
    public func updateGiftVales(giftValues: [String: Int]) {
