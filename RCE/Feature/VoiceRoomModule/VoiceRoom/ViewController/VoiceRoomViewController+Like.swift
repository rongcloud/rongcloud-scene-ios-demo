...
fileprivate final class VoiceRoomLikeTapGesture: UITapGestureRecognizer, UIGestureRecognizerDelegate {
    ...
    public var tapInterval: TimeInterval = 0.25
...
extension VoiceRoomViewController {
    ...
    private func showLikeIcons() {
        ...
        let tempView: UIView = currentUserRole() == .creator ? toolBarView.settingButton : toolBarView.messageButton
        
        let layer = VoiceRoomLikeIconLayer("like")
        view.layer.addSublayer(layer)
        
        let tempFrame = view.convert(tempView.frame, from: toolBarView)
