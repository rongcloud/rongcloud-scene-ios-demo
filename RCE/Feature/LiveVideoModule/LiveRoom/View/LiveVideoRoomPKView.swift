...
class LiveVideoRoomPKView: UIView {
    private lazy var muteButton: UIButton = {
        ...
        instance.setImage(R.image.disable_remote_audio_icon(), for: .selected)
    ...
    private lazy var otherRoomButton: UIButton = {
        ...
        instance.setImage(R.image.room(), for: .normal)
    ...
    @objc private func handleOtherRoomDidClick() {
        ...
        SVProgressHUD.show()
        let api = RCNetworkAPI.roomInfo(roomId: PK.otherRoomId())
        networkProvider.request(api) { [weak self] result in
            ...
            case let .success(model):
                guard
                    let room = model.data,
                    let controller = self?.controller?.parent,
                    let container = controller as? RCRoomContainerViewController
                else { return }
                container.switchRoom(room)
