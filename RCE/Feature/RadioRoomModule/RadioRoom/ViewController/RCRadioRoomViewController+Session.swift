...
extension RCRadioRoomViewController {
    private func dispatch_join(complation: @escaping (Result<Void, ReactorError>) -> Void) {
        let room = roomInfo
        queue.async {
            ...
            /// 更新用户所属房间，静默操作
            networkProvider.request(.userUpdateCurrentRoom(roomId: room.roomId)) { _ in }
    ...
    private func dispatch_leave(_ complation: @escaping (Result<Void, ReactorError>) -> Void) {
...
extension RCRadioRoomViewController {
    ...
    private func listenToTheRadio(_ completion: @escaping (Result<Void, ReactorError>) -> Void) {
...
extension RCRadioRoomViewController: RCRadioRoomOwnerViewProtocol {
    func masterViewDidClick() {
        ...
        if roomKVState.seating {
            ...
            let navigation = RCNavigation.masterSeatOperation(userId, isMute, self)
...
extension RCRadioRoomViewController: RCRadioRoomKVDelegate {
    ...
    func roomKVDidChanged(speaking: Bool) {
        ...
        seatInfo.status = .using
        seatInfo.isMuted = roomKVState.mute
        RCRoomFloatingManager.shared.setSpeakingState(isSpeaking: speaking, seatInfo: seatInfo)
    }
