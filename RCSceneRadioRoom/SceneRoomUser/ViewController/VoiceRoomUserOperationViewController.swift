...
class UserOperationViewController: UIViewController {
    ...
    @objc private func setManager() {
        ...
        radioRoomService.setRoomManager(roomId: roomId, userId: userId, isManager: setManager) { [weak self] result in
    ...
    @objc private func handleFollow() {
        let userId = dependency.userId
        let follow = !isFollow
        radioRoomService.follow(userId: userId) { [weak self] result in
            switch result.map(AppResponse.self) {
            case let .success(res):
                if res.validate() {
